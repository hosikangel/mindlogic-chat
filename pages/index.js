...
export default function Home() {
  useEffect(() => {
    const PERSONAS = [
      { id: 'default',   emoji: '🤖', name: 'MindLogic AI' },
      { id: 'counselor', emoji: '💆', name: '감성 상담사' },
      { id: 'tutor',     emoji: '📚', name: '학습 튜터' },
      { id: 'creative',  emoji: '✍️', name: '창작 파트너' },
      { id: 'advisor',   emoji: '💼', name: '비즈니스 어드바이저' },
    ]
    ...
    let messages     = []
    let ragContext   = localStorage.getItem('ml_rag') || ''
    let isLoading    = false
    let modelList    = [] // 동적으로 불러온 모델 목록
    ...
    function updateRagIndicator() {
      if (ragContext.trim()) {
        ...
        const len = ragContext.length
        ragIndicator.querySelector('.rag-size').textContent =
          len > 1000 ? (len/1000).toFixed(1)+'k자' : len+'자'
      } else {
    ...
    document.getElementById('rag-btn').addEventListener('click', () => {
      document.getElementById('rag-text-input').value = ragContext
      document.getElementById('rag-modal').classList.remove('hidden')
    })

    ;['api-modal','model-modal','rag-modal'].forEach(id => {
    ...
    document.getElementById('rag-file-input').addEventListener('change', async (e) => {
      ...
      document.getElementById('rag-text-input').value = text
      document.getElementById('rag-filename').textContent = `📄 ${file.name} (${(file.size/1024).toFixed(1)}KB)`
    })

    document.getElementById('save-rag-btn').addEventListener('click', () => {
      ragContext = document.getElementById('rag-text-input').value.trim()
      localStorage.setItem('ml_rag', ragContext)
      document.getElementById('rag-modal').classList.add('hidden')
      updateRagIndicator()
      if (ragContext) appendSystem(`📚 RAG 문서 적용됨 — <strong>${ragContext.length}자</strong>`)
      else appendSystem('📚 RAG 문서가 초기화되었습니다.')
    })

    document.getElementById('clear-rag-btn').addEventListener('click', () => {
      document.getElementById('rag-text-input').value = ''
      document.getElementById('rag-filename').textContent = ''
    })

    ragIndicator.addEventListener('click', () => {
      document.getElementById('rag-text-input').value = ragContext
      document.getElementById('rag-modal').classList.remove('hidden')
    })
    ...
    async function sendMessage() {
      ...
      try {
        const res = await fetch('/api/chat', {
          ...
          body: JSON.stringify({ apiKey, messages, model: currentModel, ragContext: ragContext || undefined })
        })
        const data = await res.json()
        hideTyping()
        if (data.error) {
  ...
  const css = `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{--bg:#0f0f13;--bg2:#18181f;--bg3:#22222d;--accent:#6c63ff;--accent2:#a78bfa;--text:#f0f0f5;--text2:#9090a8;--border:rgba(255,255,255,0.07);--radius:20px;--safe-bottom:env(safe-area-inset-bottom,0px)}
    ...
    .logo-mark{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-family:'Space Mono',monospace;font-size:12px;font-weight:700;color:#fff;letter-spacing:-1px;flex-shrink:0}
    ...
    .header-btn:active{background:#2a2a38}
    ...
    #rag-indicator{display:none;align-items:center;gap:6px;padding:6px 14px;background:rgba(108,99,255,0.1);border-bottom:1px solid rgba(108,99,255,0.2);cursor:pointer;flex-shrink:0}
    .rag-dot{width:7px;height:7px;border-radius:50%;background:var(--accent);flex-shrink:0}
    .rag-label{font-size:11px;color:var(--accent2);flex:1}
    .rag-size{font-size:11px;color:var(--text2)}
    ...
    .avatar{width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0}
    ...
    #send-btn{width:36px;height:36px;border-radius:10px;background:var(--accent);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform .15s,opacity .15s}
    ...
    .modal-save{width:100%;background:var(--accent);border:none;border-radius:12px;padding:13px;color:#fff;font-size:15px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif}
    ...
    .model-item.selected{border-color:var(--accent);background:rgba(108,99,255,0.08)}
    ...
    .model-check{color:var(--accent);font-size:16px;font-weight:700;flex-shrink:0}
    .rag-textarea{width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:11px 13px;color:var(--text);font-size:13px;font-family:'Noto Sans KR',sans-serif;margin-bottom:10px;outline:none;resize:vertical;min-height:120px;max-height:260px;transition:border-color .2s}
    .rag-textarea:focus{border-color:var(--accent)}
    .rag-file-btn{display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--bg3);border:1px dashed rgba(108,99,255,0.4);border-radius:12px;cursor:pointer;color:var(--text2);font-size:13px;margin-bottom:10px;transition:border-color .2s}
    .rag-file-btn:hover{border-color:var(--accent);color:var(--accent2)}
    #rag-filename{font-size:12px;color:var(--accent2);margin-bottom:10px;min-height:16px}
    .btn-row{display:flex;gap:8px}
    .welcome-msg{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 20px;text-align:center;gap:10px}
    .welcome-logo{width:60px;height:60px;border-radius:18px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-family:'Space Mono',monospace;font-size:20px;font-weight:700;color:#fff}
    ...
    <>
      ...
      <div id="app">
        <header>
          ...
          <button className="header-btn" id="rag-btn"   title="RAG 문서">📚</button>
          <button className="header-btn" id="key-btn"   title="API 키">⚙️</button>
        </header>

        <div id="persona-bar"></div>

        <div id="rag-indicator">
          <div className="rag-dot"></div>
          <span className="rag-label">📚 RAG 문서 적용 중</span>
          <span className="rag-size"></span>
        </div>

        <div id="messages">
          <div className="welcome-msg" id="welcome">
            <div className="welcome-logo">ML</div>
            <div className="welcome-title">MindLogic Chat</div>
            <div className="welcome-sub">🧠 버튼을 눌러 사용 가능한 모델과<br/>커스텀 챗봇을 불러오세요.</div>
          </div>
        </div>
      ...
      {/* RAG 모달 */}
      <div id="rag-modal" className="modal-overlay hidden">
        <div className="modal-sheet">
          <div className="modal-handle"></div>
          <div className="modal-title">📚 RAG 문서 설정</div>
          <div className="modal-desc">AI가 참고할 문서를 추가하세요.<br/>파일 업로드 또는 직접 붙여넣기 모두 가능합니다.</div>
          <label className="rag-file-btn">
            <span>📎</span>
            <span>파일 선택 (TXT, MD, CSV, JSON 등)</span>
            <input type="file" id="rag-file-input" accept=".txt,.md,.csv,.json,.js,.py,.html,.css" style={{display:'none'}} />
          </label>
          <div id="rag-filename"></div>
          <div className="modal-label">또는 직접 붙여넣기</div>
          <textarea className="rag-textarea" id="rag-text-input" placeholder="참고할 내용을 여기에 붙여넣으세요...&#10;&#10;예) 학교 규정, 강의 자료, 업무 매뉴얼 등"></textarea>
          <div className="btn-row">
            <button className="modal-secondary" id="clear-rag-btn" style={{flex:1}}>🗑️ 초기화</button>
            <button className="modal-save" id="save-rag-btn" style={{flex:2}}>적용하기</button>
          </div>
        </div>
      </div>
    </>
