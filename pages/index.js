    ...
    <>
      <Head>
        ...
        <style>{`
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          :root {
            --bg: #0f0f13; --bg2: #18181f; --bg3: #22222d;
            --accent: #6c63ff; --accent2: #a78bfa;
            --text: #f0f0f5; --text2: #9090a8;
            --user-bubble: #6c63ff; --bot-bubble: #22222d;
            --border: rgba(255,255,255,0.07); --radius: 20px;
          ...
          .header-btn { background: var(--bg3); border: 1px solid var(--border); color: var(--text2); width: 36px; height: 36px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 16px; transition: background 0.2s; flex-shrink: 0; }
          ...
          .modal-input { width: 100%; background: var(--bg3); border: 1px solid var(--border); border-radius: 12px; padding: 12px 14px; color: var(--text); font-size: 14px; font-family: 'Space Mono', monospace; margin-bottom: 16px; outline: none; }
          .modal-input:focus { border-color: var(--accent); }
          .modal-save { width: 100%; background: var(--accent); border: none; border-radius: 12px; padding: 14px; color: #fff; font-size: 15px; font-weight: 700; cursor: pointer; font-family: 'Noto Sans KR', sans-serif; }
      ...
      <div id="api-modal" className="hidden">
        <div className="modal-sheet">
          ...
          <div className="modal-desc">MindLogic API 키를 입력해 주세요.<br/>키는 기기에만 저장됩니다.</div>
          ...
          <div className="modal-label">Persona ID (선택)</div>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        function renderPersonaList() {
          ...
          list.innerHTML = PERSONAS.map(p =>
            ...
            '<div class="persona-info"><div class="p-name">' + p.name + '</div><div class="p-desc">' + p.desc + '</div></div></div>'
          ).join('');
          list.querySelectorAll('.persona-item').forEach(el => {
            el.addEventListener('click', () => {
              currentPersona = PERSONAS.find(p => p.id === el.dataset.id);
              document.getElementById('persona-label').textContent = currentPersona.name;
              renderPersonaBar(); renderPersonaList();
            });
          });
        }
        ...
        document.getElementById('save-key-btn').addEventListener('click', () => {
          ...
          personaId = document.getElementById('persona-id-input').value.trim() || null;
        ...
        if (!apiKey) setTimeout(() => document.getElementById('api-modal').classList.remove('hidden'), 600);
        ...
        function appendMessage(role, text) {
          ...
          const avatarBot = '<div class="avatar">' + currentPersona.emoji + '</div>';
          const avatarUser = '<div class="avatar" style="background:var(--bg3);color:var(--text2);font-size:12px;">나</div>';
          div.innerHTML =
            (role === 'bot' ? avatarBot : '') +
        ...
        function showTyping() {
          ...
          div.innerHTML = '<div class="avatar">' + currentPersona.emoji + '</div>' +
            '<div class="msg-wrap"><div class="bubble" style="padding:0;"><div class="typing-dots"><span></span><span></span><span></span></div></div></div>';
          msgContainer.appendChild(div);
          msgContainer.scrollTop = msgContainer.scrollHeight;
        }
        ...
        async function sendMessage() {
          ...
          try {
            const res = await fetch('/api/chat', {
              ...
              body: JSON.stringify({ apiKey, messages, personaId: currentPersona.persona_id || personaId || undefined })
            });
            const data = await res.json();
            hideTyping();
            if (data.error) { appendMessage('bot', '❌ 오류: ' + data.error); }
            else {
